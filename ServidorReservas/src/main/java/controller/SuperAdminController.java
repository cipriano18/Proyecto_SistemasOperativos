...
/**
 * Controlador encargado de gestionar los Super Administradores y el estado del
 * servidor.
 *
 * <p>
 * Permite:
 ...
 *
 * <ul>
 * <li>Crear Super Administradores</li>
 * <li>Asignar o revocar privilegios</li>
 * <li>Encender y apagar el servidor</li>
 * <li>Administrar la visualización de contraseñas</li>
 * </ul>
 *
 * @author Cipriano
 */
public class SuperAdminController {
    ...
    public void initialize() {
        ...
        roleColumn.setCellValueFactory(
                new PropertyValueFactory<>("idRole")
        );

        roleColumn.setCellFactory(col
                -> new TableCell<User, Integer>() {
        ...
        passwordVisible.textProperty().addListener(
                (obs, oldValue, newValue) -> {
    ...
    private void handleTogglePassword() {
        ...
        if (showingPassword) {
            ...
            togglePasswordBtn.getStyleClass().add(
                    "active"
            );
            ...
            togglePasswordBtn.getStyleClass().remove(
                    "active"
            );
        }
    }
    ...
    /**
     * Inicia el servidor y el servicio de limpieza.
     */
    ...
    /**
     * Detiene el servidor y el servicio de limpieza.
     */
     ...
     *
     * @param serverOn estado actual del servidor
     */
    private void setServerState(boolean serverOn) {
        ...
        if (serverOn) {
            ...
            adminContent.getStyleClass().remove(
                    "locked-section"
            );

            sectionTitleLabel.getStyleClass().remove(
                    "locked-section"
            );
            ...
            serverButton.getStyleClass().remove(
                    "btn-primary"
            );

            serverButton.getStyleClass().add(
                    "btn-danger"
            );

        } else {

            if (!adminContent.getStyleClass().contains(
                    "locked-section"
            )) {

                adminContent.getStyleClass().add(
                        "locked-section"
                );
            }

            if (!sectionTitleLabel.getStyleClass().contains(
                    "locked-section"
            )) {

                sectionTitleLabel.getStyleClass().add(
                        "locked-section"
                );
            }
    ...
    /**
     * Asigna privilegios de Super Administrador.
     */
    @FXML
    private void handleSelectAdmin() {

        User selected
                = adminTable.getSelectionModel()
                        .getSelectedItem();
    ...
    /**
     * Revoca privilegios de Super Administrador.
     */
    @FXML
